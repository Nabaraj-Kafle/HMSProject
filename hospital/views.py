...
from .models import Doctor, Patient  # Ensure correct import of models
from django.contrib.auth.decorators import login_required

# Your helper functions to check user type
def is_admin(user):
    return user.is_staff  # or any custom logic for admin check

def is_doctor(user):
    return hasattr(user, 'doctor')  # Example, assuming you have a related Doctor model

def is_patient(user):
    return hasattr(user, 'patient')  # Example, assuming you have a related Patient model
...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        # Check if the doctor account is approved
        accountapproval = Doctor.objects.filter(user_id=request.user.id, status=True)
        if accountapproval.exists():  # Check if any approved doctor exists
            return redirect('doctor-dashboard')
        else:
            return render(request, 'hospital/doctor_wait_for_approval.html')
    
    elif is_patient(request.user):
        # Check if the patient account is approved
        accountapproval = Patient.objects.filter(user_id=request.user.id, status=True)
        if accountapproval.exists():  # Check if any approved patient exists
            return redirect('patient-dashboard')
        else:
            return render(request, 'hospital/patient_wait_for_approval.html')

    # If none of the conditions are met (like user type not found)
